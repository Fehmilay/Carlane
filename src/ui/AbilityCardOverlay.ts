...
export class AbilityCardOverlay implements Screen {
  ...
  render(): void {
    ...
    const y = r.h / 2 - 78 - (1 - Ease.outBack(Math.min(1, this.t * 4))) * 24;
    drawBezel(r, x, y, w, 152, P.ink);
    ...
    r.disc(r.w / 2, y + 58, 18 + pulse * 2, mix(ab.color, P.black, 0.6));
    r.ring(r.w / 2, y + 58, 18 + pulse * 3, ab.color, 1, 0.8);
    drawIcon(r, ab.icon, r.w / 2 - 8, y + 50, P.white);
    // car + description
    const spr = sideSprite(this.v);
    r.sprite(spr, r.w / 2, y + 96, { scale: Math.min(1.1, (w - 40) / spr.w) });
    r.text(L(ab.desc), r.w / 2, y + 102, { align: 'center', color: P.white, wrap: w - 24, lineHeight: 9 });
    this.ok.rect.y = y + 124;
    this.ok.draw(r);
  }
